...
const createBill = async (IdAcc, Name, Sdt, TotalPrice, Address, Destination, Date, Note, IdPay, StatusPay, result) => {
    await db.query('INSERT INTO bill (IdAcc, Name, Sdt, TotalPrice, Address, Destination, Status, DATETIME, Note, PayMent,StatusPay)  VALUE(?,?,?,?,?,?,?,?,?,?,?)',[IdAcc,Name,Sdt,TotalPrice,Address,Destination,0,Date,Note,IdPay,StatusPay], function(err,data){
        if(err || data.length <= 0) {
            result (null)
        }
        else {
...
const getAllPriceDate = async (result) => {
    await db.query(`SELECT DATE_FORMAT(datetime, '%Y-%m') AS month, SUM(totalPrice) DIV 1000 AS total_price FROM bill GROUP BY month ORDER BY month;`, function(err,data){
        if(err || data.length <= 0) {
            result (null)
        }
        else {
